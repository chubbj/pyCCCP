...
### JJC modified function ###
def generateCrickBB_asym(cN, chL, r0, r1, w0, w1, a, ph1, cr, dph0, zoff, 
    ...
    if angle_unit == 'degree':
        ...
        w0 = [x * np.pi / 180. for x in w0]
        w1 = [x * np.pi / 180. for x in w1]
        a = [x * np.pi / 180. for x in a]
        ph1 = [x * np.pi / 180. for x in ph1]
        dph0 = [x * np.pi / 180. for x in dph0]
    ...
    for i in range(cN):
        if cr[i]: # chain is parallel to chain 0
            ...
            if ztype == 'registerzoff':
                ...
                dz = absoluteToZoff_aa(zo, r0[i], r1[i], w0[i], w1[i], a[i], 
                                       ph1[0], ph1[i], cr[i])
            ...
            if ztype == 'apNNzoff':
                ...
                zo = zo - dz
                zoff[i] = zoff[i] + zo
        T = np.array([[np.cos(dph0[i] - zoff[i] * np.tan(a[i]) / r0[i]), 
                       np.sin(dph0[i] - zoff[i] * np.tan(a[i]) / r0[i]), 0.], 
                      [-np.sin(dph0[i] - zoff[i] * np.tan(a[i]) / r0[i]), 
                       np.cos(dph0[i] - zoff[i] * np.tan(a[i]) / r0[i]), 0.], 
                      [0., 0., 1.]])
        xyz = np.dot(xyz, T)
