...
def fitCrickBB(pdbfile, cN, pType='GENERAL', IP=[], LB=[], UB=[], mask=[], 
               selection='', bbtype='ca', angle_units='degree', trajectory=False):
    ...
    # prepare bounds for minimisation if provided
    if len(LB) > 0:
        assert len(LB) == len(UB), ValueError('Number of elements in lower/upper bounds do not match!')
    ...
    ## 20241027 JJC Start ##
    # For assemblies with a superhelical diameter greater than the chain length, the principal component (PC)
    # axis may not align correctly with the intended superhelical axis. To correct this, we compare each PC 
    # axis with the vector from the N to C terminus of the first chain. The best-aligned axis is then set as 
    # the Z-axis to ensure the structure aligns with the superhelical axis.
    
    # First helix vector (from the first to the last residue in the first chain)
    first_helix_vector = M[chL - 1] - M[0]
    first_helix_vector /= np.linalg.norm(first_helix_vector)  # Normalize for comparison

    # Calculate alignment of each principal component with the first helix vector
    alignment_scores = [np.abs(np.dot(v[:, i], first_helix_vector)) for i in range(3)]

    # Find the index of the principal component best aligned with the first helix
    best_aligned_index = np.argmax(alignment_scores)

    # If the best-aligned principal component isn't the Z-axis (index 2), swap or rotate as needed
    if best_aligned_index != 2:
        # Swap the principal component that best aligns with the Z-axis
        v[:, [2, best_aligned_index]] = v[:, [best_aligned_index, 2]]
    ## 20241027 JJC End ##
    ...
    res = minimize(crickSSD, p0, args=(M, pType, cN, co, cr, True),
                   method='L-BFGS-B', jac=True, tol=1e-8, bounds=bounds)
